...
namespace FSMS.Service.Services.WeatherServices
{
    public class WeatherService : IWeatherService
    {
                    ...
                    if (detailElements != null)
                    {
                        foreach (var detailElement in detailElements)
                        {
                            ...
                            if (!string.IsNullOrEmpty(description) && !string.IsNullOrEmpty(imageSrc))
                            {
                                ...
                                Weather weather = new Weather()
                                {
                                    WeatherName = "Thời tiết Bà Rịa Vũng Tàu trong 10 ngày tới",
                                    UserId = userId,
        ...
        {
            string pattern = @"(\w+ \d{1,2}/\d{1,2}|\d{1,2}° / \d{1,2}°|Độ ẩm \d{1,3} %|\d{1,3} %|Wind \d+(\.\d+)? km/h|Gió (\d+(\.\d+)?) km/h|Mưa (\d+(\.\d+)?) mm|\[\w+\s\d+\s\]|Áp suất (\d+(\.\d+)?) mmhg|Mặt trời mọc/lặn \d+:\d+ \d+:\d+|Áp suất \d+(\.\d+)? mmhg)";
            description = Regex.Replace(description, pattern, "[$1]");
